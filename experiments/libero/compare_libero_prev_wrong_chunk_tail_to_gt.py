...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and prev-only noise-corrupted streaming."
        )
    )
    ...
    parser.add_argument("--tail-fraction", type=float, default=1)
...
def _resolve_anchor_steps(*, args: argparse.Namespace, gt_actions: np.ndarray, action_horizon: int) -> list[int]:
    ...
    min_anchor = int(obs_stride_env_steps)
    if max_anchor < min_anchor:
        raise ValueError(
            f"No legal prev anchors available: max_anchor={max_anchor}, required minimum aligned anchor={min_anchor}."
        )
    if args.anchor_steps is not None:
        raw_anchors = [int(v.strip()) for v in str(args.anchor_steps).split(",") if v.strip()]
    else:
        legal_anchors = list(range(min_anchor, max_anchor + 1, obs_stride_env_steps))
    ...
    for step in deduped:
        if step < min_anchor or step > max_anchor:
            raise ValueError(f"Anchor step {step} out of range [{min_anchor}, {max_anchor}]")
...
def _run_streaming_anchor_chunk(
    *,
    ...
    wrong_prev_env_step = None
    if int(anchor_step) > 0:
        prev_submit_step = (int(anchor_step) - 1) // obs_stride_env_steps * obs_stride_env_steps
        if prev_submit_step >= 0:
            wrong_prev_env_step = int(prev_submit_step)
    ...
    def _selector(
        ...
        if (
            ...
            and str(mode) == "wrong"
            and wrong_prev_env_step is not None
            and int(env_step) == int(wrong_prev_env_step)
        ):
    ...
    return {
        ...
        "wrong_prev_env_step": (None if wrong_prev_env_step is None else int(wrong_prev_env_step)),
...
def _summarize_job_records(job_records: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    if not job_records:
        return []
    rows: list[dict[str, Any]] = []
        ...
        rows.append(
            {
                ...
                "trigger_env_step": int(item.get("trigger_env_step", -1)),
                "trigger_obs_index": int(item.get("trigger_obs_index", -1)),
                "job_duration_ms": float(item.get("job_duration_ms", 0.0)),
                "job_wall_ms": float(item.get("job_wall_ms", 0.0)),
                "num_step_samples": int(item.get("num_step_samples", 0)),
                "num_snapshot_copy_samples": int(item.get("num_snapshot_copy_samples", 0)),
            }
        )
    return rows
...
def _summarize_runtime_stats(runtime_stats: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        "phase_id": int(runtime_stats.get("phase_id", -1)),
        ...
        "job_records": _summarize_job_records(runtime_stats.get("job_records")),
    }
...
def main() -> None:
    ...
    for anchor_step in anchor_steps:
        print(f"[anchor-debug] compare_prev_wrong anchor={int(anchor_step)} standard begin", flush=True)
        ...
        print(f"[anchor-debug] compare_prev_wrong anchor={int(anchor_step)} standard done", flush=True)
        print(f"[anchor-debug] compare_prev_wrong anchor={int(anchor_step)} wrong begin", flush=True)
        ...
        print(f"[anchor-debug] compare_prev_wrong anchor={int(anchor_step)} wrong done", flush=True)
        ...
        anchor_details.append(
            {
                ...
                "standard_wrong_prev_env_step": standard_out["wrong_prev_env_step"],
                "wrong_wrong_prev_env_step": wrong_out["wrong_prev_env_step"],
    ...
    summary = {
        "evaluation_mode": "matched_anchor_standard_vs_prev_wrong_single_chunk",
