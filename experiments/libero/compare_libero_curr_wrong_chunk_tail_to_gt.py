...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and curr-only noise-corrupted streaming."
        )
    )
...
def _run_streaming_anchor_chunk(
    *,
    ...
    def _selector(
        ...
        if phase == "bootstrap":
            if int(anchor_step) == 0 and str(mode) == "wrong":
                bootstrap_obs, meta = _select_wrong_obs(
                    obs_history=obs_history,
                    rng=rng,
                )
                return bootstrap_obs, {"applied_source": "noise", **meta}
        ...
        if phase == "anchor_submit" and str(mode) == "wrong":
...
def main() -> None:
    ...
    for anchor_step in anchor_steps:
        print(f"[anchor-debug] compare_curr_wrong anchor={int(anchor_step)} standard begin", flush=True)
        ...
        print(f"[anchor-debug] compare_curr_wrong anchor={int(anchor_step)} standard done", flush=True)
        print(f"[anchor-debug] compare_curr_wrong anchor={int(anchor_step)} wrong begin", flush=True)
        ...
        print(f"[anchor-debug] compare_curr_wrong anchor={int(anchor_step)} wrong done", flush=True)
    ...
    summary = {
        "evaluation_mode": "matched_anchor_standard_vs_curr_wrong_single_chunk",
