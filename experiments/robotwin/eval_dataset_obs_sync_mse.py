...
from experiments.robotwin.eval_dataset_obs_mse_common import NumpyEncoder, run_sync_mse  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run synchronous FastWAM on RoboTwin dataset observations and report action MSE."
    )
...
def main() -> None:
    args = parse_args()
    result = run_sync_mse(args)
    output_path = Path(args.output_json).resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, cls=NumpyEncoder)
    print(f"Wrote RoboTwin dataset-observation sync MSE to: {output_path}")
