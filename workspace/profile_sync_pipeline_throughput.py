...
def run_sync_throughput_profile(
    *,
    ...
    total_step_stage_ms: List[float] = []
    total_action_stage_ms: List[float] = []
    pushed_steps = 0
    produced_chunks = 0
    effective_actions = 0
    nominal_actions = 0
    ...
    try:
        with torch.inference_mode():
            ...
            while time.perf_counter() < end_time:
                ...
                ts_ms = int(1000.0 * (step_start - rollout_start))
                ...
                infer_dt_ms = 1000.0 * (time.perf_counter() - infer_start)
                total_step_stage_ms.append(infer_dt_ms)
                latencies_ms.append(1000.0 * (time.perf_counter() - step_start))
                ...
                if not isinstance(action_chunk, torch.Tensor):
                    raise RuntimeError("sample_and_schedule must return tensor action_chunk")
                nominal_actions += int(action_chunk.shape[1])
                effective_actions += int(action_chunk.shape[1])
                total_action_stage_ms.append(infer_dt_ms)

                # True synchronous control: finish the whole predicted chunk
                # before starting the next observation / inference cycle.
                target_time = time.perf_counter() + int(action_chunk.shape[1]) * (float(source_dt_ms) / 1000.0)
            ...
            return {
                ...
                "sync_stage_ms_summary": _summary(total_step_stage_ms),
                "sync_action_ms_summary": _summary(total_action_stage_ms),
            }
    finally:
        env.close()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Profile synchronous RTC pipeline throughput.")
    ...
    metrics = run_sync_throughput_profile(
        pipeline=pipeline,
        cfg=cfg,
