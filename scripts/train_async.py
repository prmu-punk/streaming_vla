...
def main() -> None:
    ...
    """
    parser = argparse.ArgumentParser(description="Run RTC async training")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/train_libero90_async.yaml",
        help="Path to training config (relative to repo root or absolute).",
    )
    parser.add_argument(
        "--run-name",
        type=str,
        default="rtc_async_run",
        help="Hydra run name under outputs/runs.",
    )
    parser.add_argument(
        "--extra",
        type=str,
