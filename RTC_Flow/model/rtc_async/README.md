...
- 顶层聚合导出：`model.rtc_async.__init__`
  ...
  - 调度：`RTCChunkScheduler`、`schedule_rtc_chunk`
  - 训练 RTC：`build_rtc_inpainting_batch`、`rtc_velocity_loss`、`build_token_rtc_label_mask`
  - Qwen3 流式适配：`RTCQwen3StreamAdapter`、`export_selected_kv_cache`
  - 兼容入口：`RTCVLAEntry`
...
- `qwen3_stream/`
  ...
  - `RTCQwen3StreamAdapter.export_condition_snapshot`：一次性导出 KV/attention/token_log/step_spans 条件快照。
...
- `training/`
  ...
  - `build_token_rtc_label_mask`：token 路径下对动作 token 前缀打 `-100`。

- `compat/`
  - `RTCVLAEntry`：统一构建入口，避免业务层直接拼装各子模块。
...
- `qwen3_stream/stream_runner_snapshot.py` ↔ `qwen3_stream/stream_runner.py`
  - snapshot 负责“写入与维护上下文”，adapter 负责“导出条件视图”。
- `compat/vla_entry.py` ↔ 顶层 `__init__.py`
  - 前者聚合构建，后者统一导出，便于替换接入点与测试注入。
...
2. 用 `RTCQwen3StreamAdapter.export_condition_snapshot()` 导出选层 KV。
